...
E02-01 到 E02-04 共享同一个项目：`e02_service/`。不得为每一页重新创建 `FastAPI()`、独立 `TASKS` 或静态 metrics。

2026-07-18 验证环境：Python 3.13，29 项测试通过。reference 还必须通过服务端 principal、scope、
跨 owner 404、owner-scoped metrics、身份字段 422、跨层 request ID、Problem Details、readiness/依赖
与限流恢复、deadline 无幽灵写、幂等创建、CAS 更新和 OpenAPI 契约；准确测试数以当次 pytest 输出为准。
...
-> E02-03 GET /metrics
-> E02-04 lifecycle / reliability / concurrency contracts
```
...
| [[E02-04 请求生命周期可靠性与并发契约]] | middleware/DI/service、Problem Details、cursor pagination、readiness、deadline、idempotency、CAS | 同一 request ID 跨层；分页不泄露；失败可恢复；超时无写入；重复/并发结果可判定 |
