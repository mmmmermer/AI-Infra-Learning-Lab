...
取消与 lease/outbox fencing 的可执行子集位于 `e04_runtime_reference`：

```powershell
cd 40_实验练习/E04_Agent实验/e04_runtime_reference
.\.venv\Scripts\python.exe -m pytest -q tests\test_cancellation_and_injection.py tests\test_approval_workflow.py
```

该命令不证明完整异步队列、跨进程 worker 或 P03 集成，只证明本地确定性契约。
