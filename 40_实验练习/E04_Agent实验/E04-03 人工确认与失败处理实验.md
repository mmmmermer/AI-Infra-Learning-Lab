...
| `task_cancelled` | 任意未产生已知副作用的步骤 | 否 | `cancelled` | 显式用户/操作员终止；不是 failure |
...
取消请求只接受服务端资源路径、`expected_version` 和可选理由：
...
```python
def cancel_task(principal, task_id, request, now):
    task = select_task_for_update(principal.tenant_id, principal.user_id, task_id)
    require_version(task, request.expected_version)
    require_non_terminal(task)

    outbox = select_outbox_for_update(task.approval_id)
    if outbox and outbox.effect_started:
        raise Conflict("side effect execution already started")

    cancel_pending_approval(task.approval_id, now)
    cancel_pending_or_claimed_outbox(task.approval_id)
    update_task(task, status="cancelled", version=task.version + 1)
...
理由原文不能进入 step log/trace。worker 在 planner 前、tool 返回后和持久化下一步前重新检查 task
版本/终态；看到 `cancelled` 立即停止，通用异常处理器不得再写 `task_failed`。取消与外部副作用并发
时必须有共享存储执行栅栏：取消先提交则 `effect_started` CAS 失败且 handler 不执行；开始栅栏先
提交则硬取消返回冲突。此时即使 `effect_executed=false`，外部结果也可能未知，任务只能继续幂等
finalize、对账或进入补偿，不能为了 UI 好看返回假成功。
...
- [ ] 取消使用 task version CAS，并原子关闭 pending approval 与未执行 outbox/claim。
- [ ] 取消和失败都有带 `current_step` 的事件；取消理由原文不进入 trace。
- [ ] 已知副作用执行后取消返回冲突，不把已发生的外部动作伪装成已取消。
