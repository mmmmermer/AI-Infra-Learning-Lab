...
reference 同时加入两条独立信任边界：导入请求不能自报 tenant、collection、permission group 或
来源版本，这些字段由可信 collection policy 赋值；检索到的正文即使包含“忽略系统指令”等内容，
也只能进入 `untrusted_retrieved_data` context，不能拼接进 system instruction。后者是结构不变量，
不是对某个真实模型抗注入能力的结论。
...
3. retrieval-only reference 没有工具执行器或网络 egress，不能因文档文字产生副作用。
4. 审计 JSON 只含 query/subject hash、长度、ACL 指纹和计数，不含 query、正文、user ID 或 token。
...
- [ ] 审计记录不包含原始 query、文档正文、user ID 或 bearer credential。
