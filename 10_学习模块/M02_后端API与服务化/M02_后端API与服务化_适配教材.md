...
- RFC 9457 / RFC 9110：Problem Details、条件请求、幂等与重试相关 HTTP 语义
...
| 第一轮产物 | E02 的创建、查询、CAS 更新、动态 metrics、liveness/readiness；对应 Pydantic、request context、server-owned principal、service/repository DI、Problem Details 和并发契约测试。 |
| 核心路径 | 第 1-10 章建立 HTTP、模型、分层、测试和授权，第 11-13 章完成请求生命周期、可靠性恢复与 deadline/幂等/CAS。 |
| 查阅支线 | 第 14 章安排顺序，第 15 章集中验收，第 16 章提供来源索引；生产 OAuth/JWT、数据库事务、分布式限流和持久队列留给后续模块。 |
...
| 章节 | 类型 | 阅读方式 |
|---|---|---|
| 第 1-13 章 | `instructional` | HTTP/API、分层、授权、请求生命周期、可靠性与并发写入契约 |
| 第 14、16 章 | `appendix` | 学习导航与来源索引，按需查阅 |
| 第 15 章 | `workbook` | reference 检查与 learner reproduction 验收，二者证据分开记录 |
...
- 理解 middleware、dependency、route、service、repository 的职责边界，并用 app factory 注入隔离 repository。
- 从测试用 bearer 凭据形成服务端 `Principal`，让创建、查询和 metrics 使用同一 owner/tenant 边界。
- 区分 401、403、404、422，并用负向测试证明客户端不能覆盖身份、scope 或资源所有权。
- 用 Problem Details 稳定错误 code，让 request ID 跨入口、认证、service、repository 和错误响应。
- 让 readiness 反映依赖/连接容量，让限流和依赖错误有明确恢复；用 deadline、幂等键和版本 CAS 约束并发写入。

暂时不深入：

- 生产级身份提供方、完整 OAuth/JWT、密钥轮换和复杂用户系统。
- 微服务治理、网关、服务发现、跨副本限流/熔断平台；本章只实现单进程固定窗口 limiter。
...
| 200 | 成功 | 查询任务成功 |
| 201 | 已创建 | 创建任务成功 |
| 400 | 请求元数据非法 | request ID、deadline 或 If-Match 格式错误 |
| 404 | 找不到资源 | 查询不存在的任务 |
| 409 | 当前请求与既有操作冲突 | 幂等键已用于不同 payload |
| 412 | 条件不满足 | CAS 更新使用了旧版本 |
| 422 | 请求数据不合法 | 字段类型或约束不满足 |
| 429 | 速率限制 | 当前窗口容量耗尽，可稍后重试 |
| 503 | 暂时不可服务 | 依赖不可用或连接容量耗尽 |
| 504 | deadline 超时 | 提交前预算已耗尽 |
| 500 | 服务端错误 | 未处理异常 |
...
E02 v0.3 让 service 抛出稳定 domain-to-API error，再由统一 handler 生成 Problem Details：

```python
def get_task(task_id: str, context: RequestContext) -> TaskRecord:
    task = repository.get_for_principal(
        task_id,
        context.principal,
...
未知与跨 owner 对象的 `status/code/type` 相同，request ID 随请求变化：
...
客户端按 status 和稳定 `code` 分支，不匹配可变的英文 detail。测试还应断言 content type 为
`application/problem+json`，body/header 的 request ID 相同，且错误正文不回显原始秘密输入。
...
理解机制的人会这样判断：HTTP 状态码本来就是给客户端区分结果类型的，不能用空对象代替错误类型。
当前接口把“找不到任务”写成 404，并让 `code` 与 request ID 可被机器和日志共同关联：

```python
response = client.get("/tasks/not-found", headers={"X-Request-ID": "trace-001"})
assert response.status_code == 404
assert response.json()["code"] == "task_not_found"
assert response.json()["request_id"] == response.headers["x-request-id"]
```

第一轮不要求你枚举所有生产错误，但必须养成这个习惯：**已知失败变成明确 status/code，未知异常
只返回通用 500；日志保留受控异常类型与 `文件名:行号:函数名`，但不记录可能含输入或凭据的 exception
message。异常不能被 `{}` 吃掉，也不能把原文发给客户端或日志。**
...
e02_service/
  app/
    main.py          app factory、middleware、exception handler 与薄 route
    context.py       request_id、principal、单次计算的 deadline
    dependencies.py  AppContainer 与 FastAPI dependency providers
    errors.py        稳定 AppError，不依赖 HTTP 框架细节
    models.py        请求/响应、Problem Details、health schema
    security.py      fixture bearer -> server-owned Principal 与 scope
    service.py       授权、编排、deadline 与错误翻译
    repository.py    owner scope、connection lease、idempotency、CAS
    rate_limit.py    可注入 monotonic clock 的固定窗口 limiter
  tests/
    test_api.py
    test_lifecycle_reliability.py
    test_openapi.py
```

E02 v0.3 已提取 service，因为 route 之外已经出现授权、deadline、幂等与 repository 异常翻译等
可独立测试规则。没有再创建空的 `routes/` 包或 interface hierarchy；文件按真实职责增长，而不是
按框架模板凑层数。
...
| 层 | 长期责任 | E02 当前状态 |
|---|---|---|
| middleware/context | request ID、deadline 和响应关联 | `main.py` / `context.py` |
| dependency | 构造 principal、service、limiter 与 request scope | `security.py` / `dependencies.py` |
| route | HTTP body/header/status 与速率限制入口 | `main.py` |
| API schema | 请求、响应、health 与错误结构 | `models.py` |
| service | scope、编排、deadline 和 domain error 翻译 | `service.py` |
| repository | owner-scoped 原子读写、容量、幂等、CAS | 内存 reference 已实现 |
...
```mermaid
flowchart LR
  Middleware["middleware: request context"] --> Dependency["DI: principal + service"]
  Dependency --> Route["route: HTTP contract"]
  Route --> Service["TaskService"]
  Service --> Repo["owner-scoped repository"]
  Repo --> Memory["per-process memory"]
  Service -. future .-> Queue["durable queue / worker"]
```

本次提取 service 的触发证据是这些规则已经出现：

- 同一 scope 规则必须保护 API 和直接 service 调用。
- repository unavailable/capacity/deadline/version 异常需要统一翻译成稳定 API code。
- 创建需要协调服务端 ID/时间、payload fingerprint、idempotency key 和提交 deadline。
- app factory 要为每个测试注入独立 repository/clock/limiter，不能依赖全局状态碰巧为空。

后续替换数据库时，route/service 契约应保持，repository 的内存原子段才迁移为事务和条件更新。
当前 app factory 证明依赖可替换，不证明真实数据库 adapter 已存在。
...
- 查询不存在任务返回 404 Problem Details，稳定 `code == "task_not_found"`，request ID 与 header 一致
- 创建后能查询同一条任务
...
| 未知任务 | `GET /tasks/not-found` | 404，`code=task_not_found`，Problem Details 与 request ID |
...
当前 `test_api.py` 预期摘要是 `11 passed`；再加 16 项 lifecycle/reliability/concurrency 与 2 项
OpenAPI 契约，全套为 `29 passed`。新增用例覆盖认证、scope、伪造身份字段、跨 owner/tenant 404、
owner-scoped metrics、request ID、可靠性恢复和并发写入。学习者
...
```text
E02 Task API 0.3.0
['/livez', '/metrics', '/readyz', '/tasks', '/tasks/{task_id}']
['200', '201', '400', '401', '403', '404', '409', '412', '422', '429', '500', '503', '504']
False
```
...
还要检查引用能否解析。Pydantic 的独立 JSON Schema 可能包含 `#/$defs/...`；若把它原样内联到
OpenAPI response，`$ref` 会从整份 OpenAPI 根解析并指向不存在的位置。E02 在嵌入前展开局部定义，
测试再递归收集所有 `$ref`，按 JSON Pointer 从文档根逐个解析。看到 `/docs` 能打开不等于 schema
没有悬空引用。
...
- `TaskRecord.required` 精确包含 `task_id/task_type/priority/estimated_duration_ms/status/created_at/version`。
- priority 的 JSON Schema 上下界是 1 和 10，时长上下界是 0 和 `86_400_000`。

验收：单独运行 OpenAPI 测试得到 `2 passed`，全套 E02 得到 `29 passed`。测试还必须遍历全部 4xx/5xx，
确认 OpenAPI 只声明运行时真实使用的 `application/problem+json`，并包含通用 500。任意删除
`extra="forbid"`、改变边界、改错错误媒体类型、制造悬空 `$ref` 或重新加入未评审路径时，OpenAPI
契约测试必须失败。
...
`Principal`，同一 request context 经 route/service 传入 repository，使空状态变为含一个属于该 principal 的 `pending` 任务，创建响应可被本人
...
PATCH /tasks/{task_id} + If-Match -> 条件更新
GET /metrics -> 查询指标
GET /livez, /readyz -> 区分进程存活与依赖就绪
```
...
accDescr: 请求先建立 request context 和服务端 Principal，再经 route、service 按 owner 写入内存 repository，查询和 metrics 使用相同所有权约束。
  Client --> Middleware["middleware: request_id + deadline"]
  Middleware --> Auth["dependency: credential -> Principal"]
  Auth --> Route["route: HTTP contract"]
  Route --> Service["TaskService"]
  Service --> Repo["TaskRepository + owner scope"]
  Repo --> Get["GET /tasks/{task_id} + same Principal"]
  Repo --> Metrics["GET /metrics + same Principal"]
```

这张图描述的是 E02 当前真实链路；虚线外的持久数据库、队列和 worker 尚未实现。
...
下面是 E02 v0.3 路由的职责片段。middleware、handler、service 和 repository 完整定义见 reference；
片段不应脱离这些依赖单独复制：
...
from .dependencies import get_request_context, get_task_service
from .models import TaskCreate, TaskRecord
...
- `RequestContext` 只从 middleware 与 principal dependency 建立；请求体没有身份、request ID 或权限字段。
- `TaskService` 控制 scope、服务端 ID/UTC 时间、deadline 与 domain error 翻译。
- 首次创建返回 201；同 key/payload 的幂等 replay 返回 200 和同一记录。
- repository 在 owner scope 内原子写 task 与 idempotency mapping。
- `ETag` 暴露当前 version，后续 PATCH 使用 `If-Match` 做 CAS。
- 统一 exception handler 生成稳定 Problem Details，而不是每个 route 手写不同错误 body。
...
所以重复运行只能检查整体确定性，不能单独证明“后置 clear 必然执行”。v0.3 已增加 app factory
与 `AppContainer`，lifecycle 测试会注入独立 repository 并断言默认全局 repository 未变化；基础
`test_api.py` 仍保留 autouse clear，用来展示两种测试隔离层级。
...
当前已验证 reference 的基线是 **29 项测试通过**（11 个基础 API、16 个 lifecycle/reliability/
concurrency、2 个 OpenAPI schema），其中包含认证、scope、跨 owner 隔离、owner-scoped metrics、
request ID、依赖/限流恢复、deadline、幂等和 CAS。这个数字只证明
...
M02 负责保证：任务能进来、状态能查、指标能看，并且重复/超时/并发更新的 API 结果可判定。
...
随机 UUID 只降低被猜中的概率，不证明调用者有权访问这条任务。E02 v0.3 使用短小、明确标成
...
认证、授权和对象可见性；第 11-13 章继续处理生命周期、可靠性与并发写入；第 14 章再收束学习顺序；
...
E02 的 `alice-fixture`、`bob-fixture`、`carol-fixture` 和 `reader-fixture` 是故意使用短字符串的非秘密测试凭据。
...
这样 Alice 创建的任务即使 ID 被同租户的 Bob 或另一租户的 Carol 得知，两者仍只能得到与未知 ID
相同的 `404 task_not_found`。列表和 metrics 也按相同 principal 聚合，避免单条资源不泄露、集合或
总数却泄露。Carol 的 tenant 只能来自服务端 credential 映射，不能由请求字段指定。
...
```powershell
python -m pytest tests/test_api.py tests/test_lifecycle_reliability.py -q
```
...
| 另一 tenant 的 Carol 读取 Alice 的单项/列表/metrics | 404 / 空列表 / 全零 | 响应与日志不含双方身份原值 |
...
owner-scoped repository 决定对象是否可见。四层必须按顺序成立。下一章把 principal 放进不可变
request context，并用同一个 request ID 贯穿 service、repository、错误响应和脱敏日志。

---

## 第 11 章：请求生命周期、Middleware 与依赖注入

### 11.1 本章目标与前置

前十章分别讲了 route、schema、repository 和 principal，但真实请求不是这些文件的列表。完成本章
后，你应能沿一个 request ID 解释请求如何穿过入口、认证、service、repository 与错误响应，并用
测试而不是示意图证明链路成立。

前置是 M01 第 13 章的 task/timeout 心智模型和本模块第 6、10 章。当前 reference 不使用 OpenTelemetry，
request ID 是日志关联键而不是分布式 trace；跨进程 trace 留给 M08。

### 11.2 入口只计算一次请求作用域状态

E02 middleware 做三件有限的事：

1. 接受符合 `[A-Za-z0-9._-]{1,64}` 的 `X-Request-ID`，缺失时生成；非法值返回 400，避免控制字符进入日志。
2. 把 `X-Request-Deadline-Ms` 的剩余预算在入口用 monotonic clock 转成绝对 deadline；后续层不得重新给满预算。
3. 在响应 header 写回同一 request ID，并记录不含 headers/body 的 entry 日志。

<!-- textbook-code: role=fragment env=e02-reference network=off -->
```python
request.state.request_id = incoming_id or uuid4().hex
request.state.deadline_at = container.clock() + deadline_ms / 1000
response = await call_next(request)
response.headers["X-Request-ID"] = request.state.request_id
```

反例是在 route、service 和 repository 各自生成 UUID。每条日志看似“有 trace”，但三个 ID 无法关联。
另一个反例是逐层读取 `X-Request-Deadline-Ms` 并从当前时间重新加 10ms：请求每经过一层都会续期，
deadline 实际失效。

### 11.3 Dependency 负责构造可信对象，不承载业务写入

FastAPI dependency graph 在每次请求中构造：

```text
HTTPBearer -> get_principal(request) -> immutable Principal
request.state + Principal -> RequestContext
request.app.state.container -> TaskService / limiter
```

`Principal` 的 tenant/user/scopes 只来自服务端 fixture 映射，请求 body 仍由 `extra="forbid"` 拒绝
身份字段。`AppContainer` 保存 repository、clock、limiter、ID/时间 factory；`create_app(container)`
让测试注入独立实现。测试创建 isolated repository 后 POST，并同时断言 isolated count 为 1、默认
global count 为 0，这才证明 DI 生效，而不是 fixture 恰好先 clear 全局状态。

Dependency 不应在解析 principal 时创建任务或修改 repository。认证、构造依赖和业务副作用分开，
才能断言 401/403/422 发生时没有写入。

### 11.4 Route 薄，不等于 service 只做转发

route 负责 HTTP 特有信息：body/header schema、mutation rate limit、动态 status 和 ETag。service
负责 scope、服务端字段、deadline 与 domain error 翻译。repository 负责 owner-scoped 原子读写。

```text
POST /tasks
-> middleware(request_id, deadline)
-> get_principal + get_request_context
-> route(rate limit, Idempotency-Key)
-> service(scope, payload fingerprint, orchestration)
-> repository(connection lease, owner, atomic commit)
```

这条分工允许 service 单元在没有 HTTP 的情况下复用授权与 deadline 规则，也允许将内存 repository
替换为数据库 adapter。把所有规则搬进 dependency 会让依赖解析产生隐藏副作用；把所有规则留在
route 则使 worker/CLI 直接调用时绕过授权和并发契约。

### 11.5 request ID 如何跨成功与错误路径

`test_request_id_crosses_entry_security_service_repository_and_error` 发送固定合法 ID，访问不存在任务，
再从 `caplog.records` 收集同一 ID 的 stage。集合必须包含：

```text
entry, security, service, repository, error_response
```

响应 Problem Details 的 `request_id` 与 `X-Request-ID` header 也必须等于输入。测试同时扫描日志记录，
禁止出现 fixture bearer、`Authorization`、tenant 和 user 原值。principal 只以
`sha256(tenant:user)[:12]` 的 `subject_ref` 进入日志。

哈希摘要不是匿名化承诺：低熵主体空间仍可能被枚举。它只是在当前 reference 中减少原值传播；生产
日志还需访问控制、保留期、密钥化伪名或合规策略。错误 handler 也不返回 exception 原文，避免内部
路径、SQL 或输入秘密进入客户端响应。

同一边界也适用于服务端日志：`logger.exception()` 会连同 traceback 输出异常 message，而下游异常
文本可能嵌入 URL 凭据、SQL 参数或输入片段。E02 未知异常只记录异常类型和不含源码文本的
`文件名:行号:函数名` 栈帧位置；测试向异常 message 注入 fixture bearer、Authorization 与秘密值，
并同时断言响应和日志都不包含这些值。

### 11.6 Worked example、反例与验收

运行聚焦测试：

```powershell
python -m pytest tests/test_lifecycle_reliability.py::test_request_id_crosses_entry_security_service_repository_and_error -q
python -m pytest tests/test_lifecycle_reliability.py::test_app_container_injects_an_isolated_repository -q
```

独立练习：

1. 成功创建时使用 `trace-learner-001`，断言 response header 和所有 stage 一致。
2. 发送含空格或换行的 request ID，断言 400 `invalid_request_id` 且日志未出现原值。
3. 新建只读 principal，断言 403 前后 repository count 相同，日志不含 credential。
4. 画出 dependency graph，并指出把客户端 `owner_id` 放进 `RequestContext` 会破坏哪条信任边界。

验收必须同时包含成功、错误、无副作用和脱敏断言。只检查响应 header 不足以证明 service/repository
收到了同一上下文；只检查日志没有字符串 `token` 也不足以证明真实 Authorization 值未泄露。

### 11.7 权威来源

- [FastAPI Middleware](https://fastapi.tiangolo.com/tutorial/middleware/)
- [FastAPI Dependencies](https://fastapi.tiangolo.com/tutorial/dependencies/)
- [Starlette Request state](https://www.starlette.io/requests/)
- [OWASP Logging Cheat Sheet](https://cheatsheetseries.owasp.org/cheatsheets/Logging_Cheat_Sheet.html)

---

## 第 12 章：Problem Details、分页与可恢复过载

### 12.1 稳定 code 比匹配 detail 更可靠

RFC 9457 Problem Details 给错误提供共同骨架。E02 使用 URI `type`、标题、HTTP `status`、可读
`detail`，并扩展稳定 `code`、`request_id`、可选 `retry_after_ms` 和脱敏 `invalid_params`。

客户端应按 status/code 决策：401 重新认证，409 检查幂等 key/payload，412 重新读取版本，429/503
按恢复提示退避，504 不盲目假定写入已回滚。英文 detail 可以改进措辞，不应成为程序分支协议。

Pydantic validation error 常包含原始 input；E02 handler 只保留 `loc` 和 `type`，测试用
`do-not-echo-this-value` 作为未知 owner 字段，并断言响应文本不含它。这是最小不回显边界，不替代
全面敏感字段分类。

### 12.2 集合 API 必须有有界、owner-scoped 分页

`GET /tasks?limit=2&cursor=...` 返回：

```json
{
  "items": [{"task_id": "...", "version": 1}],
  "next_cursor": "opaque-base64url-value"
}
```

limit 约束为 1-100。cursor 编码上一页最后一个 task ID，repository 先按 principal 过滤、稳定排序，
再定位 cursor 和截取下一页。解码器只接受无填充 Base64URL 字符集，并在 decode 后重新编码；只有
完全相同的 canonical 字符串才有效，因此多余字符、`=` 和非 canonical pad-bit 别名都返回 400。
Alice 的 cursor 不能让 Bob 看见 Alice 记录；错误、过期或不可见 cursor 统一 400 `invalid_cursor`。

反例是先全局分页再按 owner 过滤：某页可能少项，cursor 进度和总量还会泄露其他 owner 的存在。
另一个反例是无上限返回全部任务，数据量增长后会放大内存、序列化和响应超时。当前 base64 cursor
只是不透明展示，不是签名防篡改 token；生产 cursor 应绑定查询条件/owner，并考虑签名和过期。

### 12.3 Liveness 与 readiness 回答不同问题

`/livez` 只回答“进程事件循环/线程池还能响应”，不访问依赖；`/readyz` 回答“现在是否应接收业务
流量”，会尝试非阻塞取得 repository connection lease。

| 情况 | livez | readyz | 动作 |
|---|---|---|---|
| repository unavailable | 200 | 503 `dependency_unavailable` | 暂停新流量，依赖恢复后重试 probe |
| pool capacity exhausted | 200 | 503 `dependency_capacity_exhausted` | `Retry-After`，释放 lease 后恢复 |
| 进程无法响应 | probe 失败 | 无法调用 | 由进程/平台重启策略处理 |

固定 200 readiness 会在依赖故障时继续吸收请求；把短暂依赖故障变成 liveness 失败又会触发重启风暴。
E02 用 `pool_size=1`、测试线程先持有唯一 lease 的方式确定性复现耗尽，释放后不重启 app 即恢复。
另一个 hook 在写提交前切断依赖：请求必须 503、repository 不变，恢复后同一 app 再次写入成功。
它不证明真实数据库连接泄漏、排队 timeout 或多进程 pool 已测试。

### 12.4 429 与 503 都要给出恢复路径

mutation endpoint 按 `subject_ref` 使用固定窗口 limiter。manual clock 测试容量 1：首次成功、第二次
429 `rate_limit_exceeded`，带 `Retry-After`/`retry_after_ms`；推进一个窗口后再次成功。测试不使用
sleep，因此快速、确定。

429 表示调用者超过服务策略，503 表示服务依赖/容量暂不可用；两者都可能重试，但策略不同。客户端
应使用有上限、带 jitter 的退避并尊重总 deadline，不能所有请求在整秒边界同时重试。当前 fixed
window 有边界突发，且状态只在单进程内；生产多副本全局限流尚未完成。

### 12.5 Worked example、练习与验收

```powershell
python -m pytest tests/test_lifecycle_reliability.py -k "validation or readiness or rate_limit or pagination" -q
```

独立练习：创建 5 条本人任务，以 limit 2 读取 2/2/1 三页，ID 不重不漏；让另一个 owner 使用第一页
cursor，断言没有记录泄露；分别制造 unavailable 与 capacity exhausted，恢复后都回到 200；将 limiter
窗口推进前后各请求一次，保存 429/成功的机器 code 和 headers。

验收要求错误 body、status、content type、request ID 和恢复动作都有断言。只测“最终又成功了”而
不检查中间 code，或用真实 sleep 等待窗口，都不能稳定定位契约。

### 12.6 权威来源

- [RFC 9457 Problem Details](https://www.rfc-editor.org/rfc/rfc9457)
- [RFC 9110 Retry-After and status semantics](https://www.rfc-editor.org/rfc/rfc9110)
...
## 第 13 章：Deadline、幂等创建与版本 CAS

### 13.1 三种失败不能用“再发一次”统一处理

写请求可能在客户端等待时超时、在网络中重复、或与另一更新并发。若服务没有显式契约：

- timeout 后客户端不知道第一次是否写入，重试可能生成第二条任务；
- 相同创建被代理/客户端重复发送，每次都返回新 ID；
- 两个调用者都读取 version 1，再写不同值，后写者静默覆盖先写者。

E02 分别用 deadline commit check、idempotency key 和 compare-and-swap 解决这三种最小情况。它们互补，
不能用一个 request ID 或随机 UUID 代替。

### 13.2 Deadline 只计算一次，并在提交前再检查

客户端发送 `X-Request-Deadline-Ms` 剩余预算。middleware 用 monotonic clock 计算绝对 deadline，
service 进入操作前检查，repository 在持锁、修改 dict 前再次检查：

<!-- textbook-code: role=fragment env=e02-reference network=off -->
```python
if deadline_at is not None and clock() >= deadline_at:
    raise DeadlineExpired
self._tasks[task.task_id] = owned_task
```

无幽灵写测试注入 manual clock：入口预算 10ms，`before_commit` 推进 20ms。创建必须是 504、
`count_all() == 0`，同幂等 key 随后重试仍是首次 201；更新必须保持原 priority/version。只在 route
开始检查会漏掉等待连接/锁期间的耗时；先写入再返回 504 则制造“响应失败、状态成功”的幽灵写。

monotonic clock 用于持续时间，不能序列化给另一台机器当全局时间。跨服务传播应传 deadline/budget
并逐跳收缩。客户端断开检测和底层同步 I/O 强制停止尚未进入 reference；当前验收只证明显式 header
deadline 的提交边界。

### 13.3 Idempotency key 绑定 owner 与 payload

repository 的幂等作用域是 `(tenant_id, user_id, key)`，值保存 payload fingerprint 和 task ID，且
检查、写任务、写映射都在同一锁内：

| 请求 | 响应 | repository 变化 |
|---|---|---|
| 首次 key + payload A | 201，replayed=false | 新增 1 条 |
| 同 key + payload A | 200，同 task ID，replayed=true | 不变 |
| 同 key + payload B | 409 `idempotency_key_conflict` | 不变 |

只按 key 不按 owner 会让租户互相冲突；只缓存 response 不比较 payload 会把调用者的新意图静默当作
旧请求；先写 task 后再写幂等表会在崩溃点留下无法重放的一半状态。内存锁只证明单进程原子性；生产
必须依靠数据库 unique constraint/transaction，并定义 key TTL 与响应保留策略。

自动反例同时覆盖两条轴：barrier 并发放行的同 owner 请求只提交一次；Alice、同租户 Bob 和另一租户
Carol 使用同一 key 则分别首次创建。只测顺序 replay 不能证明并发原子性，只测不同 tenant 又不能证明
同 tenant 的 owner 边界。

### 13.4 If-Match 版本实现原子条件更新

创建返回 `version=1` 和 `ETag: "1"`。客户端 PATCH 时发送 `If-Match: "1"`，repository 在 owner
检查与同一锁内比较 current/expected，成功后写 version 2；旧版本返回 412 `version_conflict`。

并发测试用两个 TestClient 同时以 version 1 更新 priority。最终 status 必须恰好是 `[200, 412]`，
成功响应和后续 GET 都是 version 2。测试不依赖谁赢，只断言不变量，因此不会用 sleep 猜线程顺序。

反例是在 route 先读 version、释放锁，再调用普通 update；两个请求都可能通过检查。数据库实现应把
条件写进同一 SQL，例如 `UPDATE ... WHERE id=? AND owner=? AND version=?`，并用 affected row count
判断 CAS，而不是应用层先查后写。

### 13.5 Worked example、练习与验收

```powershell
python -m pytest tests/test_lifecycle_reliability.py -k "deadline or idempotent or concurrent" -q
```

独立练习：并发发送两次同 key/payload，断言 count 仍为 1；先更新到 version 2，再用旧 `"1"` 和新
`"2"` 各 PATCH，断言前者 412、后者成功到 version 3；把 deadline hook 改为未超时，证明成功路径
仍提交一次。每个负例都要断言 repository 状态，而不是只看 status。

完成口径：超时无写、幂等 replay/conflict、并发 CAS 三类测试全部独立通过；能解释内存锁到数据库
transaction/unique/conditional update 的迁移。不得宣称客户端断开取消、跨副本幂等或数据库事务已完成。

### 13.6 权威来源

- [RFC 9110 Conditional Requests and If-Match](https://www.rfc-editor.org/rfc/rfc9110)
- [Python 3.13 `time.monotonic`](https://docs.python.org/3.13/library/time.html#time.monotonic)
- [Google SRE: Handling Overload](https://sre.google/sre-book/handling-overload/)
- [Stripe idempotent requests](https://docs.stripe.com/api/idempotent_requests)：生产 API 的具体参考，
  不是 HTTP 通用标准。
...
- 画出当前 middleware -> dependency -> route -> service -> repository 链路和未来 durable queue/worker 扩展
...
- 让同一 request ID 进入 middleware、security、service、repository 和错误响应
- 用 app factory/container 注入独立 repository
- 断言日志不含 bearer、Authorization、tenant/user 原值和完整 body

### 第 9 天：错误、分页与过载恢复

目标：

- 使用 Problem Details 的稳定 code，而不是匹配 detail
- owner-scoped cursor 分页不重不漏，坏/跨 owner cursor 不泄露记录
- 区分 livez/readiness，复现依赖 unavailable、pool exhaustion 和 429 后恢复

### 第 10 天：Deadline、幂等与 CAS

目标：

- deadline 在入口计算一次，提交前过期不产生幽灵写
- 同 key/payload 重放同一创建，同 key/不同 payload 返回 409
- 两个旧版本更新恰好一个成功，另一个 412

实验：

- [[40_实验练习/E02_后端API实验/E02-04 请求生命周期可靠性与并发契约]]
...
- middleware、dependency、route、service、repository 分别拥有哪类状态和规则？
- 为什么接口测试比手工点文档更可靠？
...
- 为什么 500 detail、英文异常文本或日志 message 不适合作为机器错误 code？
- 为什么先全局分页再按 owner 过滤会泄露或跳过数据？
- livez、readyz 和业务 dependency error 分别回答什么问题？
- 为什么 deadline 要在提交前再检查，`task.cancel()` 又不能自动回滚同步写入？
- 幂等键与版本 CAS 分别解决重复创建和并发覆盖中的哪一种问题？
...
Python 应为 3.13，测试摘要应为 `29 passed`：11 个基础 API、16 个 lifecycle/reliability/concurrency、
2 个 OpenAPI schema。这一步只证明仓库参考实现可执行，不自动勾选 learner reproduction。

### 15.2 Learner reproduction 验收

只有同时满足下列条件，才能标记为本人完成：

- [ ] 跑起一个 FastAPI 服务
- [ ] 亲手实现创建、cursor 列表、单项查询、If-Match 更新、动态 metrics、livez 与 readyz
...
- [ ] 未知任务返回 404 Problem Details；稳定 `code=task_not_found`，body/header request ID 一致
...
- [ ] 同一 request ID 跨 entry/security/service/repository/error；日志不含凭据、身份原值或完整 body
- [ ] cursor 分页有 1-100 上限、不重不漏，跨 owner/bad cursor 统一失败且不泄露记录
- [ ] unavailable、pool exhausted、429 都有机器 code、Retry-After 和无需重启的恢复断言
- [ ] deadline 在提交前耗尽返回 504 且 repository 不变
- [ ] 同幂等 key/payload 只创建一次；payload 冲突 409；作用域不跨 owner
- [ ] 两个相同 If-Match 并发更新恰好一个 200、一个 412，最终 version 只增加一次
- [ ] 第 7.7 节九行契约都有自动断言，而不是只看终端或 `/docs`
- [ ] 能解释 `TaskCreate` 和 `TaskRecord` 为什么要分开
- [ ] 能说明内存 repository 会在重启和多进程下失效
- [ ] 能画出持久数据库、队列和 owner/tenant 约束的下一步位置，但不把数据库事务、跨副本幂等/限流、客户端断开取消冒充成 E02 已实现功能
...
- RFC 9457 Problem Details：https://www.rfc-editor.org/rfc/rfc9457
- RFC 9110 HTTP Semantics / If-Match / Retry-After：https://www.rfc-editor.org/rfc/rfc9110
