...
| 版本边界 | 当前可执行基线为 Python 3.13；E06 SQLite reference v0.3.0；P03 service v0.3.1 使用 Python 3.13.2、PostgreSQL 17.5 和 Redis 8.0.3。P03 的运行契约是 Redis Streams consumer group；RQ/Celery 只用于概念对照 |
...
> 2026-07-19 校订：`create task -> enqueue -> mark queued` 是不安全双写。最低正确模型改为同一数据库事务写入 `tasks + outbox`，由可重试 dispatcher 投递；worker 使用幂等键、compare-and-set、lease、heartbeat 和 reconciliation。可执行 reference 位于 `E06/e06_sqlite_reference/`，当前 42 个测试覆盖数据库/队列语义、owner-scoped FastAPI、worker 闭环和安全 cache-aside。缓存 backend 为确定性内存/故障替身，不是实际 Redis 集群证据。
...
## 第 5 章：Redis 缓存与安全 cache-aside

### 5.1 本章目标

学完本章后，应当能从一次请求完整推导缓存行为，而不只是会写 `GET/SET`：

1. 先认证和授权，再决定可检索的 source scope。
2. 用服务端事实生成稳定、隔离且可失效的 key。
3. 在 miss 时只允许一个 leader 计算，其他并发请求共享结果。
4. 把过期、淘汰、损坏值和缓存服务不可用变成显式观测值。
5. 缓存失败时回源同一条授权路径，不把“降级”写成“跳过权限”。

Redis 在这里是 cache backend，不是任务、权限或文档版本的唯一事实来源。E06 的确定性 reference 使用同一 backend contract 下的内存 LRU/TTL 实现和故障注入实现；它验证控制流，不声称执行过真实 Redis 集群测试。

### 5.2 先画对顺序：cache key 不是授权检查
...
-> tenant / owner / effective ACL authorization
-> AuthorizedScope（允许参与候选生成的 source ids + 数据版本）
-> cache key
-> cache lookup
-> validated hit 或 authorized compute
```

不能把顺序写成：

```text
query -> 全局 cache hit -> 响应前再过滤 source
```

后一种写法已经让越权文档参与了命中、排序或答案生成。即使最终删掉 source 字段，答案文本仍可能泄露私有内容。E06 reference 因此在每次 hit 前重新执行 `authorize()`，并再次验证缓存值和新计算结果中的 `source_ids` 都属于本次 `AuthorizedScope`。

### 5.3 什么适合缓存，什么仍以数据库为准

| 对象 | 可缓存原因 | 必须进入 key 或校验的事实 | 事实来源 |
|---|---|---|---|
| embedding | 相同规范化文本重复向量化成本高 | model、normalizer、text hash；读取前已获文档权限 | 模型/文档配置 |
| RAG 检索结果 | 相同授权上下文和查询可能短时间重复 | tenant、owner、ACL、source scope、collection、数据/索引/模型/检索器版本、filter、top-k、query | 身份系统、文档库、索引配置 |
| 任务临时进度 | 前端轮询频繁 | task id、owner、task version、短 TTL | `tasks` 表 |
| metrics 临时聚合 | 重复聚合成本高 | metric schema、窗口、label scope | 原始指标/事件 |

`tasks.status`、`result_json`、`error_type` 和任务事件链仍以数据库为准。缓存丢失最多造成 miss 和额外延迟，不能造成任务消失或权限扩大。

### 5.4 可复算的 retrieval key
...
先把 tuple、filter 和键顺序规范化，再用 UTF-8、无多余空白的 canonical JSON 求 SHA-256。hash 用于固定长度和避免分隔符歧义，不会把错误的上下文变正确。漏掉某个影响结果的字段，仍会产生错误命中。

### 5.5 version invalidation 与 TTL 分工

失效有两种不同机制：

| 机制 | 解决的问题 | 不保证什么 |
|---|---|---|
| 版本进入 key | 更新后新请求不再命中旧语义 | 不立即释放旧条目空间 |
| TTL/显式 delete | 限制旧条目保留时间和内存 | 不能代替版本或授权隔离 |

例如 `document_version` 从 `docs-v1` 变成 `docs-v2` 后，新请求自然生成新 key 并 miss；`docs-v1` 条目可以等 TTL 或 LRU 淘汰。若只执行 delete 而 key 中没有版本，删除与下一次并发写入之间仍可能重新放回旧结果。

TTL 也不是越长越好。应根据文档更新频率、计算成本、撤权传播要求和可接受陈旧时间选择，并分别记录：
...
### 5.6 stampede：为什么普通 cache-aside 仍会过载

普通写法在一个热门 key 同时过期时会发生：

```text
8 个请求同时 miss
-> 8 次 embedding/retrieval
-> 8 次写缓存
```

single-flight 把同一进程、同一 key 的并发 miss 合并为一个 flight：

```text
leader: miss -> compute -> set -> publish result
waiters: wait for leader -> share result
```

> 代码角色：pseudocode（解释 single-flight 所有权，不可直接运行）。

```python
scope = authorize(principal, request)  # 每个调用者都先授权
key = make_key(principal, scope, request)

if validated := cache.get(key):
    return validated

future, is_leader = flights.join_or_create(key)
if not is_leader:
    return future.result()

try:
    result = compute(scope, request)
    assert set(result.source_ids) <= set(scope.source_ids)
...
E06 的测试让 8 个线程同时进入同一 key，并要求 `compute_calls == 1`。这个结论只覆盖单进程 reference；跨进程或多实例需要 Redis 锁、请求折叠服务或其他协调方案，还要处理 leader 崩溃和锁租约，不能直接外推。

### 5.7 损坏值、淘汰和 Redis 不可用

cache hit 也不是可信结果。reference 的 value envelope 固定 `schema_version`、`answer` 和 `source_ids`；出现未知字段、错误类型、未知 schema 或越权 source 时执行：

```text
decode/validation failed
-> delete bad entry when possible
-> authorized recompute
-> cache_value_invalid = true
```

内存 backend 用有界 LRU 防止条目无限增长。生产 Redis 还需要明确 `maxmemory` 与 eviction policy，并观察 eviction rate；否则“设置了 TTL”仍可能在到期前耗尽内存。

缓存连接失败时：

```text
authorize 已完成
-> cache adapter 报 CacheBackendUnavailable
-> 用同一个 AuthorizedScope 回源计算
-> cache_unavailable = true
-> 任务可以 succeeded
```

只捕获 adapter 明确定义的可用性错误。程序 bug、序列化不变量失败或结果越权不能被宽泛 `except Exception` 吞掉后当作正常降级。

### 5.8 worked example：六次请求逐步推导

固定 query=`RAG why sources`、collection=`demo_docs`、top-k=2：

| 次序 | 变化 | 预期 | 原因 |
|---:|---|---|---|
| 1 | public / acl-v1 / docs-v1 | miss，compute 1 次 | 冷启动 |
| 2 | 完全相同 | hit | 全部结果与授权维度相同 |
| 3 | user 改为 compliance | miss | owner、permission groups 与 ACL fingerprint 改变 |
| 4 | public 的 docs 改为 v2 | miss | version invalidation |
| 5 | v2 条目写入私有 source 的损坏值 | invalid + recompute | hit 后 source scope 校验失败 |
| 6 | backend unavailable | authorized compute + error metric | 缓存故障不改变授权范围 |
...
测试实现位于 `e06_reference/cache.py`。`MemoryCacheBackend` 与 `UnavailableCacheBackend` 是确定性测试替身，不是 Redis 性能、网络超时或集群 failover 证据。

### 5.9 机制型反例：高命中率怎样变成越权事故

错误 key=`rag:{collection}:{query_hash}`。compliance 用户先查询并缓存 `doc-private`；public 用户随后命中同一 key。若系统只在响应末尾删除 `retrieved_sources`，答案中仍可能保留私有条款。

修复不是“再加一个 permission_group 字符串”，而是同时满足：

1. principal 只能由服务端认证层构造；客户端身份字段被 schema 拒绝。
2. 授权过滤先于候选生成和缓存读取。
3. owner、有效 ACL、ACL version 和实际 source scope 都参与 key。
4. hit 与 compute 结果都要验证 source subset。
5. Redis 故障只能回源同一授权 scope。

### 5.10 独立练习

1. 把 `top_k=2` 改为 5，预测 embedding cache 与 retrieval cache 各自 hit/miss，再用测试验证。
2. 为 `filter=(region=cn)` 增加一个未见输入，证明 filter 改变时 key 改变。
3. 删除 `document_version`，先写 v1 再请求 v2，写出能够触发陈旧命中的失败测试；修复后保留测试。
4. 让 compute 返回一个不在 `AuthorizedScope` 的 source，确认请求失败且污染值不会写入 cache。
5. 把 8 线程测试改为两个不同 key，验收每个 key 各计算一次，而不是全局串行。

### 5.11 本章检查标准

- [ ] 能画出 principal -> authorization -> key -> cache -> compute 的正确顺序。
- [ ] 能解释 tenant、owner、ACL、source fingerprint 和各版本为何进入 key。
- [ ] 能区分 version invalidation、TTL、delete 和 eviction。
- [ ] 能用 `compute_calls` 证明同 key 并发 miss 有界。
- [ ] 能让损坏值被删除并重算，而不是作为 hit 返回。
- [ ] 能让缓存不可用时继续使用同一授权 scope，并记录降级指标。
- [ ] 能说明 E06 内存/fault backend 的证据边界，不把它写成真实 Redis 验证。

### 本章来源

- [Redis caching](https://redis.io/solutions/caching/)：cache-aside、TTL 与缓存用途。
- [Redis key eviction](https://redis.io/docs/latest/develop/reference/eviction/)：`maxmemory` 与淘汰策略。
- [Redis client-side caching](https://redis.io/docs/latest/develop/use/client-side-caching/)：失效通知与版本边界。
- [Redis distributed locks](https://redis.io/docs/latest/develop/clients/patterns/distributed-locks/)：跨进程协调的额外前提；本章不把单进程 single-flight 等同于分布式锁。
