...
| 核心路径 | 第 1-10 章建立模型、函数、模块、异常、IO、OOP 和测试能力，第 12 章组装 CLI，第 13 章建立并发选择与取消；第 11 章用于变式诊断。 |
| 查阅支线 | 第 14 章安排顺序，第 15 章集中验收，第 16 章提供外部资料；它们不替代第 1-13 章的教学正文。 |
...
| 章节 | 类型 | 阅读方式 |
|---|---|---|
| 第 1-13 章 | `instructional` | 概念、示例、反例与工程应用主线 |
| 第 14、16 章 | `appendix` | 学习导航与来源索引，按需查阅 |
| 第 15 章 | `workbook` | 固定输入、命令和验收证据，不作为新概念首讲 |
...
- 根据阻塞 I/O、CPU 计算和原生 async 等待选择 thread/process/asyncio，并验证取消后资源释放。
...
学习者必须亲手走完“dict 排序 -> dataclass -> pytest -> JSON/CLI 工程化 -> 并发选择与取消”并保留失败修复记录，才能把 M01 标记为本人完成。E01-04 已提供并发 reference，但 reference 通过不替代本人复现；后文的 CLI 工程化扩展改记为尚待创建的 E01-05 候选任务。
...
第 13 章 并发与取消  → 为 FastAPI/worker 建立 thread/process/async 心智模型
第 14 章 学习顺序    → 把章节映射到每天的动手任务
第 15 章 学习检查    → 用固定输入、命令和阈值验收
第 16 章 外部资料    → 完成主线后按需查阅
```
...
学习者练习及未来 E01-05 的**目标结构**，不是已实现状态；作者侧跑通本章独立示例也不能替代
学习者创建这些产物。
...
这部分是选读，P01 v0.1、E01-04 并发实验和 E01-05 CLI 候选任务都不依赖外部网络。
...
这段代码展示目标分层，但当前 P01 没有 `scheduler/io.py`，E01-05 也尚未创建，因此不能复制后
...
- **E01-05 把排序脚本工程化（候选）**：建议作为后续学习者任务，覆盖模块拆分、JSON 边界、argparse、logging 和 CLI 回归；当前仓库尚未创建该实验，不能标成已完成。E01-04 已用于并发选择与取消，不得混用状态。
...
错误退出都可判定。下一步先完成独立练习；E01-05 真正建立后，再把目标结构迁移成 workbook。
...
M02 的 FastAPI 路由和后续 worker 都会出现 `async`、线程池、进程池、timeout 与 cancellation。如果
没有前置心智模型，很容易把 `async def` 当成“自动并行”，或在超时返回后让后台写入继续发生。
完成本章后，你应该能够：

- 根据工作时间花在等待还是计算上，选择 thread、process 或 `asyncio`；
- 用小型对照实验发现 event loop 中的阻塞调用，并用 `asyncio.to_thread()` 隔离同步 I/O；
- 区分发出 `cancel()` 与 task 真正结束，等待取消并检查 `finally` 资源释放；
- 解释 timeout 是取消来源，不是事务回滚机制；共享状态写入仍需在提交点检查 deadline；
- 运行 E01-04 的 6 项标准库测试，并说明小计时不能外推为生产 benchmark。

前置是 M00 第 6A 章的进程/退出模型，以及本模块第 3、6、9 章的函数、异常和测试。当前基线是
默认 CPython 3.13 构建；不讨论实验性的 free-threaded 构建、无锁算法或分布式并发。

### 13.2 先分类工作负载，再选择 API

“并发”表示多个工作在时间上有重叠，“并行”表示同一时刻实际在多个执行资源上计算。三种模型
解决的瓶颈不同：

| 工作负载 | 第一选择 | 得到什么 | 主要成本 |
|---|---|---|---|
| 阻塞文件/同步网络 I/O | `ThreadPoolExecutor` / `to_thread` | 等待时间重叠，共享同一进程内存 | 线程安全、过多线程、无法强制停止正在运行的同步调用 |
| CPU 密集纯 Python | `ProcessPoolExecutor` | 默认 CPython 下可跨进程使用多核 | 序列化、进程启动、内存复制、崩溃隔离 |
| 大量原生可等待 I/O | `asyncio` | 单线程协作调度大量 task | 所有长边界必须主动 `await`，阻塞一个即阻塞全部 |

选择规则不是“网络一定 async、计算一定 process”。现有 SDK 只有同步 API 时，线程可能是低风险
迁移；NumPy 等本地代码可能释放 GIL，应先测量；极小 CPU 任务的进程启动和序列化成本可能大于
计算本身。模型选择必须写出工作负载、库边界和测量证据。

reference 用枚举让理由可以测试，而不是散在注释里：

<!-- textbook-code: role=fragment env=python-3.13 network=off -->
```python
decision = choose_concurrency_model(WorkloadKind.BLOCKING_IO)
assert decision.model == "thread"
```

### 13.3 Thread：重叠阻塞等待，不保证 CPU 加速

E01-04 对四个 `sleep(0.03)` 工作先顺序执行，再以四线程执行。两个路径必须返回相同顺序和值，
线程路径在同一次运行中应更短。这个实验隔离的是“等待可重叠”；如果把 `sleep` 换成长纯 Python
循环，默认 CPython 线程通常不会提供同样的多核收益。

<!-- textbook-code: role=fragment env=python-3.13 network=off -->
```python
with ThreadPoolExecutor(max_workers=4) as executor:
    results = tuple(executor.map(blocking_io, items))
```

反例是只打印“快了 4 倍”。一次墙钟时间会受线程池冷启动、机器负载和计时分辨率影响。正式性能
结论需要预热、多次独立样本、分位数、固定输入/环境及 CPU/内存观察；本章断言只用于发现明显的
顺序执行回归。

### 13.4 Process：隔离 CPU 工作，同时付出序列化成本

进程池的 callable 和参数要可序列化。将 worker 函数定义在模块顶层，并在可执行脚本入口使用
`if __name__ == "__main__":`，尤其要考虑 Windows spawn 导入行为。reference 让同一个
`cpu_checksum()` 分别在进程池和当前进程运行，断言结果一致；它不声称 2,000 次循环值得启动池。

不要把打开的文件、数据库连接、锁或巨大可变对象直接传进进程池。它们可能无法序列化，或复制后
不再代表同一资源。真实 worker 更适合传稳定 ID 和小型不可变输入，再由子进程建立自己的连接。

### 13.5 Asyncio：协作调度要求每个长边界主动让出

event loop 每次运行一个 task，直到它 `await` 一个尚未完成的等待。下面虽然写在异步函数里，
`time.sleep()` 仍阻塞整个 loop：
...
`to_thread()` 不会让同步函数获得可取消事务语义：调用 task 被取消时，底层线程中已经开始的函数
可能继续运行。因此它适合有限、幂等的阻塞 I/O；涉及写入时还需要幂等键、deadline 提交检查或
数据库事务。M02 会把这条原则落实为“超时不产生幽灵写”。

E01-04 的 loop-lag 测试先安排 callback，再分别直接阻塞和 offload。修复路径显式让出一次 loop，
证明 callback 能在同步 I/O 仍执行时获得调度。它比只比较总耗时更贴近 event-loop starvation。

### 13.6 取消是一段需要等待完成的协议

`task.cancel()` 安排在下一个可取消点向 task 注入 `CancelledError`。调用返回不等于 task 已结束；
必须 await 它，才能确定 `finally` 已执行：
...
`asyncio.timeout()` 超时也通过取消当前 task 形成 `TimeoutError` 边界。资源应在 `finally` 或 async
context manager 中释放。不要无条件捕获 `CancelledError` 后继续循环；若确实要做有限清理，应在
清理后重新抛出或让 task 正常结束，并为清理本身设上限。

更复杂的多个子任务应尽量由 `TaskGroup` 等结构化作用域拥有，而不是 `create_task()` 后丢失引用。
“谁创建、谁等待、谁取消”必须可回答，否则程序退出和请求断开时容易留下孤儿工作。

### 13.7 Worked example、练习与验收
...
6 项测试分别覆盖三模型选择、thread I/O 对照、process 结果一致、event-loop blocking、显式取消和
timeout 清理。独立练习：

1. 为同步数据库驱动、纯 Python 特征计算、原生 async HTTP 客户端各写一条选择与拒绝理由。
2. 给资源探针增加 `active`，显式取消和 timeout 后都断言回到 0。
3. 新增 heartbeat，证明直接 sleep 时停顿、`to_thread` 时继续增长；不要只看打印顺序。
4. 写一个反例：调用 `cancel()` 后不 await；解释为何测试可能在 teardown 时看到 pending task。

验收要求 thread/process/async 的选择理由可被代码和文字同时检查；两条取消路径都必须
`acquired == released == 1`；计时结果只在同次运行内比较，不写成生产吞吐结论。完成后进入 M02，
在那里 request deadline、幂等创建和 CAS 会把本章的生命周期语义推进到 API 写入边界。

### 13.8 权威来源

- [Python 3.13 `asyncio` tasks, cancellation and timeout](https://docs.python.org/3.13/library/asyncio-task.html)
...
- 按第 15 章固定输入运行三种策略和非法策略

建议后续新增 **E01-05 把排序脚本工程化** 承接第 5、6 天：模块拆分、JSON 边界、argparse、logging、固定 CLI 输出和回归测试。该实验当前尚未创建，也没有 verified reference；现在只能作为 learner reproduction 的待办，不能在路线状态中标成已完成。HTTPX 继续保持选读，不属于 E01-05 验收条件。

### 第 7 天：并发选择、阻塞检测与取消

目标：

- 为阻塞 I/O、CPU 计算和原生 async I/O 分别选择 thread、process、asyncio
- 复现 event-loop blocking，并用 `to_thread` 恢复调度
- 显式取消和 timeout 后等待 task 结束，断言资源获取/释放相等

实验：

- [[40_实验练习/E01_Python基础练习/E01-04 并发选择与取消]]
...
- 阻塞 I/O、CPU 密集计算和原生 async 等待为什么不能机械使用同一种并发模型？
- 为什么 `task.cancel()` 返回后还必须 await task？`finally` 的资源释放如何验证？
- 为什么 `asyncio.to_thread()` 能恢复 event loop，但不能保证底层同步写入随取消停止？
...
### 15.3 Learner reproduction 的机器验收
...
如果已经完成尚待创建的 E01-05 候选任务，再运行 CLI：
...
三条命令退出码都应为 0，stdout 应分别与 15.2 表格完全一致。日志可以写到 stderr，但不能改变机器读取的 stdout 顺序。
...
再运行并发 reference，并在学习者变式中更换输入或资源探针：
...
预期 6 项通过；本人验收还需保存 thread/process/async 三个选择理由、一次 event-loop blocking 修复，
以及显式取消和 timeout 后资源计数归零的变式证据。

### 15.4 Reference 验证不是本人验收
...
其预期是 reference 全套测试通过。这个结果只能证明参考工程健康；不能替代学习者在自己的 E01 产物中运行第 15.3 节命令，也不能自动勾选下面的完成项。
...
- [ ] 亲手完成 E01-04 的并发选择、阻塞修复和取消清理变式；将 E01-05 明确记录为候选任务，不冒充已有 reference
