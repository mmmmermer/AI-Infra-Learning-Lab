...
版本删除。测试必须同时看到 `cache.entry_count == 0`、删除后第一次 `cache_hit == False`、结果不含
该文档，以及 v1 replay 得到 `rejected_version_conflict`。这四个断言缺一不可。

这个内存 reference 能验证状态机和调用顺序，不能证明对象存储、数据库、向量库、CDN 与备份中的
分布式删除已经完成。生产系统还需要每个 sink 的删除回执、超时补偿和审计证据。
...
- [ ] 能用“先缓存、再删除、再查询、再重放旧版本”的负例验收删除闭环。
