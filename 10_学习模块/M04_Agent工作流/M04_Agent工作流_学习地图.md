...
- [ ] 能把显式取消做成带 CAS、审计、审批/outbox 联动和副作用栅栏的终态。
- [ ] 能用默认拒绝的 URL/path validator 阻断 SSRF 与路径逃逸。
- [ ] 能证明恶意 RAG/tool output 不会扩大 capability、跳过审批或进入 trace。
...
| 执行目标安全 | 会做 exact-origin egress、root-confined path 与递归 trace 脱敏 | E04 reference 负向测试 |
...
6. 完成 E04-01/E04-02/E04-03，并把 Agent v1 字段映射到 P03 planned 接口；当前仓库不宣称已完成生产接入。
...
- [ ] 能设计人工确认节点。
- [ ] 能说明取消为什么不能只是内存 flag，以及副作用执行后为什么不能返回假取消成功。
- [ ] 能运行 SSRF、路径逃逸、恶意 tool output 与嵌套 trace 脱敏负向测试，并断言 handler/副作用调用次数。
